...
var _AppStateService = class _AppStateService {
  constructor() {
    this.localStorageKey = "dayone-ng";
    this.initialState = {
      theme: "light",
      // light, dark
      direction: "ltr",
  ...
  initializeState() {
    const state2 = __spreadValues({}, this.initialState);
    this.applyDirectionSpecificChanges(state2.direction);
    this.stateSubject.next(state2);
  }
  ...
  applyReset() {
    ...
    localStorage.clear();
    const checkedElement1 = document.querySelector('[name="theme-background"]:checked');
    if (checkedElement1) {
      checkedElement1.checked = false;
    }
  }
  updateStateAndEmit(state2) {
    const currentState = this.stateSubject.getValue();
    if (state2["theme"]) {
      this.applythemeSpecificChanges(state2["theme"]);
    }
    if (state2["direction"]) {
      this.applyDirectionSpecificChanges(state2["direction"]);
    }
    if (state2["navigationStyles"]) {
      this.applyNavigationStylesSpecificChanges(state2["navigationStyles"]);
    }
    if (state2["menuStyles"] && !state2["layoutStyles"]) {
      this.applyMenuStylesSpecificChanges(state2["menuStyles"]);
    }
    if (state2["layoutStyles"] && !state2["menuStyles"]) {
      this.applyLayoutStylesSpecificChanges(state2["layoutStyles"]);
    }
    if (state2["pageStyles"]) {
      this.applypageStylesSpecificChanges(state2["pageStyles"]);
    }
    if (state2["widthStyles"]) {
      this.applywidthStylesSpecificChanges(state2["widthStyles"]);
    }
    if (state2["menuPosition"]) {
      this.applymenuPositionSpecificChanges(state2["menuPosition"]);
    }
    if (state2["headerPosition"]) {
      this.applyheaderPositionSpecificChanges(state2["headerPosition"]);
    }
    if (state2["themePrimary"]) {
      this.applyPrimarySpecificChanges(state2["themePrimary"]);
    }
    if (state2["themeBackground"]) {
      this.applyThemeBackgroundSpecificChanges(state2["themeBackground"]);
    }
    if (state2["headerColor"]) {
      this.applyheaderColorSpecificChanges(state2["headerColor"]);
    }
    if (state2["menuColor"]) {
      this.applymenuColorSpecificChanges(state2["menuColor"]);
    }
    if (state2["backgroundImage"]) {
      this.applybackgroundImageSpecificChanges(state2["backgroundImage"]);
    }
    this.stateSubject.next(state2);
    this.updateLocalStorage(state2);
  }
  updateLocalStorage(state2) {
    try {
      localStorage.setItem(this.localStorageKey, JSON.stringify(state2));
    } catch (error) {
...
@angular/animations/fesm2022/animations.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-KTQLQ7YR.js.map
